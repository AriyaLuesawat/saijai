    ...
    <div className="flex h-[100dvh] bg-[#F0F4F8] text-slate-900 font-sans overflow-hidden">
      ...
      <aside
        className={`
          ...
          w-72 bg-[#064E3B] text-white flex flex-col shadow-2xl shrink-0
      ...
      >
        ...
        <button
          onClick={() => setSidebarOpen(false)}
          className="absolute top-4 right-4 lg:hidden text-white/60 hover:text-white text-2xl leading-none"
          aria-label="ปิด"
        >
          ×
        </button>
        ...
        <div className="p-6 flex flex-col gap-5 flex-1 overflow-y-auto">
          ...
          <div className="flex items-center justify-center bg-white/10 p-5 rounded-[2rem] border border-white/10 shadow-inner">
            <img
              ...
              alt="โลโก้ใสใจ"
              className="h-16 sm:h-20 w-auto drop-shadow-2xl hover:scale-105 transition-transform duration-300"
            />
          </div>
          ...
          <div className="bg-[#022C22] rounded-3xl p-4 border border-emerald-900">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-emerald-500 rounded-full flex items-center justify-center
                              font-black text-emerald-950 text-xs select-none shrink-0">
                AR
              </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {/* ── Top Bar ── */}
        <header className="h-16 sm:h-20 bg-white/90 backdrop-blur-md border-b border-slate-200
                           flex items-center justify-between px-4 sm:px-8 lg:px-10 shrink-0 gap-3">
          {/* Hamburger — mobile */}
          <button
          ...
          <button
            onClick={clearAll}
            className="bg-rose-50 hover:bg-rose-100 text-rose-600 px-3 sm:px-5 py-2 rounded-xl
                       text-xs font-bold transition-all border border-rose-100 whitespace-nowrap"
          >
            ...
            <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                <div className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-100
                                group hover:border-emerald-400 transition-all">
                  <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-2">ยอดใช้จ่ายรวม</p>
                  <p className="text-3xl sm:text-4xl font-black text-slate-900 group-hover:text-emerald-600 transition-colors">
                    ฿{grandTotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-100">
                  ...
                  <p className="text-3xl sm:text-4xl font-black text-slate-900">
                    {transactions.length}{" "}
                    <span className="text-base sm:text-lg font-medium text-slate-300">รายการ</span>
                  </p>
                </div>
                <div className="bg-emerald-600 p-6 sm:p-8 rounded-[2rem] text-white shadow-xl shadow-emerald-200">
                  ...
                  <p className="text-emerald-100 text-sm">
                    {transactions.length > 0 ? `หมวดยอดนิยม: ${chartData[0]?.name ?? "-"}` : "พร้อมใช้งานปกติ"}
                  </p>
                </div>
              </div>
              ...
              {/* Recent */}
              <div>
                <h3 className="font-black text-xs text-slate-400 mb-4 uppercase tracking-widest">
          ...
          {activeMenu === "upload" && (
            <div className="max-w-2xl mx-auto py-4 sm:py-8 animate-in zoom-in-95 duration-500">
              <div className="bg-white p-8 sm:p-12 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl border border-slate-50 text-center">
                <div className="w-20 h-20 sm:w-24 sm:h-24 bg-emerald-50 text-emerald-600 rounded-full
                                flex items-center justify-center text-4xl sm:text-5xl mx-auto mb-5 shadow-inner">
                  📥
                </div>
                ...
                <div
                  ...
                  onDrop={onDrop}
                  className={`relative border-4 border-dashed rounded-[2rem] p-12 sm:p-16 transition-all cursor-pointer
                    ${isDragging
                ...
                >
                  ...
                  <p className="text-emerald-700/60 font-black uppercase tracking-widest text-sm">
                ...
                {previews.length > 0 && (
                  <div className="mt-5 grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-3">
                    {previews.map((p, i) => (
                      <div
                        key={i}
                        className="relative group rounded-xl sm:rounded-2xl overflow-hidden
                                   border border-slate-100 bg-slate-50 aspect-square"
                      >
                ...
                {previews.length > 0 && (
                  <button
                    ...
                    disabled={loading}
                    className="mt-6 sm:mt-8 w-full bg-[#064E3B] text-white py-5 rounded-[1.5rem] sm:rounded-[2rem]
                               font-black text-base sm:text-lg uppercase tracking-widest hover:bg-emerald-800
                               shadow-2xl transition-all active:scale-95 disabled:bg-slate-300 disabled:cursor-not-allowed"
                  >
          ...
          {activeMenu === "history" && (
            <div className="max-w-5xl mx-auto space-y-5 animate-in fade-in duration-500">
              ...
              <input
                ...
                className="w-full px-5 py-3 rounded-2xl border border-slate-200 bg-white text-sm
        ...
        <nav className="lg:hidden border-t border-slate-200 bg-white/95 backdrop-blur-md
                        grid grid-cols-3 shrink-0">
          {NAV_ITEMS.map(({ id, icon, short }) => (
            <button
              key={id}
              onClick={() => navigate(id)}
              className={`flex flex-col items-center justify-center gap-1 py-3 text-xs font-bold
                          transition-colors
                ${activeMenu === id ? "text-emerald-600" : "text-slate-400 hover:text-slate-600"}`}
            >
